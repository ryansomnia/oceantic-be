...
let article = {
  createArticle: async (req, res) => {
    // Ambil user_id dari JWT token yang diverifikasi (req.user)
    // Jika tidak ada user, anggap token tidak valid atau middleware gagal
    const authenticatedUserId = req.user ? req.user.id : null;
    
    // Gunakan user_id dari body jika ada, atau dari JWT
    const { title, content, category, path, user_id } = req.body;
    const finalUserId = user_id || authenticatedUserId;

    let image_url = null;

    // Validasi input dasar
    // Pastikan user_id juga ada
    if (!title || !content || !category || !finalUserId) {
      return res.status(400).json({ message: 'Judul, konten, kategori, dan User ID wajib diisi.' });
    }
    ...
    try {
      const [result] = await pool.execute(
        'INSERT INTO articles (title, content, image_url, category, path, user_id) VALUES (?, ?, ?, ?, ?, ?)',
        [title, content, image_url, category, path, finalUserId] 
      );
  ...
  getArticlesByCategory : async (req, res) => {
    const { category } = req.body; // Ambil kategori dari parameter URL
  ...
  // 4. Fungsi untuk Memperbarui Artikel
   updateArticle : async (req, res) => {
    ...
    // Validasi dasar
    if (!title && !content  && !path && !category && (!req.files || !req.files.image)) {
